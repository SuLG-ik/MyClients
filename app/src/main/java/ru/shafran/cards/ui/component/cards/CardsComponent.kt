...
class CardsComponent(
    ...
    private val router =
        router<CardsConfiguration, Cards.Child>(
            initialConfiguration = CardsConfiguration.UsagesList,
            childFactory = this::createChild, handleBackButton = true
        )
    override val routerState: Value<RouterState<CardsConfiguration, Cards.Child>> =
        router.state
    ...
    private fun createChild(
        ...
        return when (configuration) {
            CardsConfiguration.Camera -> Cards.Child.Camera(
                CameraComponent(
                    componentContext,
    ...
    override fun onShowCard(cardToken: String) {
        details.onShow(cardToken)
    }
